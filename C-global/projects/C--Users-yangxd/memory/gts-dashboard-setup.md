...
description: GTS 3留测试 · 完整工作流 & Day2 生成指南（给家里 Claude）
metadata: 
  node_type: memory
  type: project
...
| 表名 | Table ID | 用途 |
|------|----------|------|
| 素材日报 | tblpZCaUwU1nQ7Jg | 23素材×3天，每日每条素材的 Meta+BI 数据 |
...
### 写作风格
- **短句**：一句一个意思，不超过 30 字
- **观点先行**：先结论再数据，不是"数据显示XX"而是"XX了，数据是YY"
- **数据只支撑观点**：不列数字清单
- **排名格式**：素材名(T层) $CPI/DNU，按出价方式分开 Install/AEO
- **禁用**：AI 套话（"值得关注""表现尚可"）、模板化结构、模糊词
- **样本不够就说样本不够**，不确定就说原因，不是因为害怕下结论

## 十、7/10（Day2）特殊要点

### 和 7/9 对比（不是和 4 月比）
- CPI 环比：涨了还是跌了（Day2 过了冷启动，应该下降）
- 花费/安装 环比
- CVR/CTR 环比

### 断投验证
周五 0-10am 暂停投放，观察花费是否在剩余时段回补。总预算模式下理论影响不大。

### 关停影响
赌场博弈双线关停后，预算是否溢出到其他素材。

### BI 数据回传
7/9 的 D1次留率(BI) 和 D3三留率(BI) 可能在 7/10 出来 → 补填 7/9 的素材日报对应字段。标记不动（用户手动改）。

### 最后一天预算倾斜
7/11 是最后一天，预算往表现好的素材倾斜冲量。

## 十一、7/9 当前状态（供参考）

- 标记：🔴关停=赌场博弈(Install+AEO)，其余🟡观察
- 问题追踪 2 条：赌场博弈双线关停、CVR商店图分析（均已解决）
- 每日结论 7/9 已填
- 简报 7/9 段落已定稿（NBRWdtCdZoQK8cxBb7JcT6BHnVg）
